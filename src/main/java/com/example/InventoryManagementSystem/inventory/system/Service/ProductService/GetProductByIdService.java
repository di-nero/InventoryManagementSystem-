...
public class GetProductByIdService {
    ...
    public ResponseEntity<Object> getProductById(Long id){
        ...
        if(result.isEmpty())
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.responseBuilder(false , "product of id " + id + " does not exist" , null));
        else {
            var data = mappers.toProductDto(result.get());
            return ResponseEntity.ok(ApiResponse.responseBuilder(true , "product found" , data));
        }
    }
